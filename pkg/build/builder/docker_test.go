...
func TestInsertEnvAfterFrom(t *testing.T) {
	...
	tests := map[string]struct {
		...
		env		[]corev1.EnvVar
		want		string
	}{"no FROM instruction": {original: `RUN echo "invalid Dockerfile"
`, env: []corev1.EnvVar{{Name: "PATH", Value: "/bin"}}, want: `RUN echo "invalid Dockerfile"
`}, "empty env": {original: `FROM busybox
`, env: []corev1.EnvVar{}, want: `FROM busybox
`}, "single FROM instruction": {original: `FROM busybox
RUN echo "hello world"
`, env: []corev1.EnvVar{{Name: "PATH", Value: "/bin"}}, want: `FROM busybox
ENV "PATH"="/bin"
RUN echo "hello world"
`}, "multiple FROM instructions": {original: `FROM scratch
FROM busybox
RUN echo "hello world"
`, env: []corev1.EnvVar{{Name: "PATH", Value: "/bin"}, {Name: "GOPATH", Value: "/go"}, {Name: "PATH", Value: "/go/bin:$PATH"}}, want: `FROM scratch
...
func TestReplaceLastFrom(t *testing.T) {
	...
	tests := []struct {
		...
		image		string
		want		string
	}{{original: `# no FROM instruction`, image: "centos", want: ``}, {original: `FROM scratch
# FROM busybox
RUN echo "hello world"
`, image: "centos", want: `FROM centos
RUN echo "hello world"
`}, {original: `FROM scratch
FROM busybox
RUN echo "hello world"
`, image: "centos", want: `FROM scratch
FROM centos
RUN echo "hello world"
`}}
...
func TestAppendPostCommit(t *testing.T) {
	...
	tests := []struct {
		...
		want		want
	}{{description: "basic multi-part bash command", original: heredoc.Doc(`
				FROM busybox
				RUN echo "hello world"
				`), postCommit: buildapiv1.BuildPostCommitSpec{Command: []string{"echo", "hello"}}, want: want{Out: heredoc.Doc(`
				FROM busybox as <alias>
				RUN echo "hello world"
				FROM <alias>
				RUN echo hello
				FROM <alias>
				`)}}, {description: "basic bash command with args", original: heredoc.Doc(`
				FROM busybox
				RUN touch /tmp/hello
				`), postCommit: buildapiv1.BuildPostCommitSpec{Command: []string{"ls"}, Args: []string{"-l", "/tmp/hello"}}, want: want{Out: heredoc.Doc(`
				FROM busybox as <alias>
				RUN touch /tmp/hello
				FROM <alias>
				RUN ls -l /tmp/hello
				FROM <alias>
				`)}}, {description: "basic bash script", original: heredoc.Doc(`
				FROM busybox
				RUN echo "hello world"
				`), postCommit: buildapiv1.BuildPostCommitSpec{Script: "echo hello $1 world"}, want: want{Out: heredoc.Doc(`
				FROM busybox as <alias>
				RUN echo "hello world"
				FROM <alias>
				RUN /bin/sh -ic 'echo hello $1 world'
				FROM <alias>
				`)}}, {description: "basic bash script with args", original: heredoc.Doc(`
				FROM busybox
				RUN echo "hello world"
				`), postCommit: buildapiv1.BuildPostCommitSpec{Script: "echo", Args: []string{"hello", "$1", "world"}}, want: want{Out: heredoc.Doc(`
				FROM busybox as <alias>
				RUN echo "hello world"
				FROM <alias>
				RUN /bin/sh -ic 'echo hello $1 world'
				FROM <alias>
				`)}}, {description: "multi-stage basic multi-part bash command", original: heredoc.Doc(`
				FROM busybox as appimage
				RUN echo "hello world"
				FROM appimage
				RUN touch /tmp/hello
				`), postCommit: buildapiv1.BuildPostCommitSpec{Command: []string{"echo", "hello", "$1", "world"}}, want: want{Out: heredoc.Doc(`
				FROM busybox as appimage
				RUN echo "hello world"
				...
				FROM <alias>
				`)}}, {description: "multi-stage basic bash command with args and alias", original: heredoc.Doc(`
				FROM busybox as appimage
				RUN echo "hello world"
				FROM appimage as myimage
				RUN touch /tmp/hello
				`), postCommit: buildapiv1.BuildPostCommitSpec{Command: []string{"echo"}, Args: []string{"hello", "$1", "world"}}, want: want{Out: heredoc.Doc(`
				FROM busybox as appimage
				RUN echo "hello world"
				FROM appimage as myimage
				RUN touch /tmp/hello
				...
				RUN echo hello $1 world
				FROM myimage
				`)}}, {description: "multi-stage basic bash script with aliases", original: heredoc.Doc(`
				FROM busybox as appimage
				RUN echo "hello world"
...
func TestDockerfilePath(t *testing.T) {
	...
	}{{dockerfilePath: "Dockerfile", dockerStrategy: &buildapiv1.DockerBuildStrategy{}}, {dockerfilePath: "mydockerfile", dockerStrategy: &buildapiv1.DockerBuildStrategy{DockerfilePath: "mydockerfile"}}, {dockerfilePath: "dockerfiles/mydockerfile", dockerStrategy: &buildapiv1.DockerBuildStrategy{DockerfilePath: "dockerfiles/mydockerfile"}}, {contextDir: "somedir", dockerfilePath: "dockerfiles/mydockerfile", dockerStrategy: &buildapiv1.DockerBuildStrategy{DockerfilePath: "dockerfiles/mydockerfile"}}}
	from := "FROM openshift/origin-base"
	expected := []string{from, "\"OPENSHIFT_BUILD_NAME\"=\"name\"", "\"OPENSHIFT_BUILD_NAMESPACE\"=\"namespace\"", "\"OPENSHIFT_BUILD_SOURCE\"=\"http://github.com/openshift/origin.git\"", "\"OPENSHIFT_BUILD_COMMIT\"=\"commitid\"", "\"io.openshift.build.commit.author\"=\"test user \\u003ctest@email.com\\u003e\"", "\"io.openshift.build.commit.date\"=\"date\"", "\"io.openshift.build.commit.id\"=\"commitid\"", "\"io.openshift.build.commit.ref\"=\"ref\"", "\"io.openshift.build.commit.message\"=\"message\"", "\"io.openshift.build.name\"=\"name\"", "\"io.openshift.build.namespace\"=\"namespace\""}
	for _, test := range tests {
		...
		build := &buildapiv1.Build{Spec: buildapiv1.BuildSpec{CommonSpec: buildapiv1.CommonSpec{Source: buildapiv1.BuildSource{Git: &buildapiv1.GitBuildSource{URI: "http://github.com/openshift/origin.git"}, ContextDir: test.contextDir}, Strategy: buildapiv1.BuildStrategy{DockerStrategy: test.dockerStrategy}, Output: buildapiv1.BuildOutput{To: &corev1.ObjectReference{Kind: "DockerImage", Name: "test/test-result:latest"}}}}}
		build.Name = "name"
		...
		dockerClient := &FakeDocker{buildImageFunc: func(opts docker.BuildImageOptions) error {
		...
		dockerBuilder := &DockerBuilder{dockerClient: dockerClient, build: build}
...
USER 1001`
	...
	build := &buildapiv1.Build{ObjectMeta: metav1.ObjectMeta{Name: "buildid", Namespace: "default"}, Spec: buildapiv1.BuildSpec{CommonSpec: buildapiv1.CommonSpec{Source: buildapiv1.BuildSource{ContextDir: "", Dockerfile: &dockerFile}, Strategy: buildapiv1.BuildStrategy{DockerStrategy: &buildapiv1.DockerBuildStrategy{DockerfilePath: "", From: &corev1.ObjectReference{Kind: "DockerImage", Name: "scratch"}}}, Output: buildapiv1.BuildOutput{To: &corev1.ObjectReference{Kind: "ImageStreamTag", Name: "scratch"}}}}}
	client := buildfake.Clientset{}
	buildDir, err := ioutil.TempDir("", "dockerfile-path")
	if err != nil {
		t.Errorf("failed to create tmpdir: %v", err)
	}
	dockerBuilder := &DockerBuilder{client: client.Build().Builds(""), build: build, dockerClient: dockerClient, inputDir: buildDir}
